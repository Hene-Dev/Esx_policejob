			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_spawner',
			{
				...
				if foundSpawnPoint then
					ESX.Game.SpawnVehicle(vehicleProps.model, spawnPoint, spawnPoint.heading, function(vehicle)
						...
						SetVehicleNumberPlateText(vehicle, "POL-" .. math.random(100,999))
...
function OpenPoliceActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'police_actions',
	{
		...
		elements = {
			...
			{label = _U('Jail'),     value = 'qwertyuio'},
		}
	}, function(data, menu)
	
	        if data.current.value == 'qwertyuio' then
			...
			'default', GetCurrentResourceName(), 'citizen_interaction',
			{
              ...
              local health = GetEntityHealth(closestPlayerPed)
				...
				Citizen.CreateThread(function()
					...
					elseif action == 'handcuff1' then
						TriggerServerEvent('esx_policejob:handcuff1', GetPlayerServerId(closestPlayer))
...
function OpenFineMenu(player)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'fine',
	{
		...
		align    = 'bottom-right',
		elements = {
			{label = 'Liikennerikkomukset', value = 0},
			{label = 'Omaisuus & ilkivalta',   value = 1},
			{label = 'Lyömä & Ampuma-ase rikkeet', value = 2},
			{label = 'Henki & vapaus rikokset', value = 3},
			{label = 'Huumausainerikokset',   value = 4},
			{label = 'Omaisuus & ilkivalta',   value = 5},
			{label = 'Ryöstöt & henkirikokset',   value = 6},
			{label = 'Siviiliin kohdistuvat',	value = 7},
			{label = 'Virkavaltaan kohdistuvat',	value = 8},
			{label = 'Luvat / Maksut',	value = 9}
		}
	}, function(data, menu)
...
AddEventHandler('esx_policejob:handcuff1', function()
	IsHandcuffed1    = not IsHandcuffed1
	local playerPed = PlayerPedId()

	Citizen.CreateThread(function()
		if IsHandcuffed1 then
...
Citizen.CreateThread(function()
	while true do
		...
		if IsHandcuffed then
			...
			DisableControlAction(0, 92,  true) --Ammunta pois autosta

			DisableControlAction(2, Keys['LEFTCTRL'], false) -- Disable going stealth
...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlJustReleased(0, Keys['F6']) and IsInputDisabled(0) and PlayerData.job ~= nil and PlayerData.job.name == 'police' and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'police_actions') then
			if Config.MaxInService == -1 then
